...
def conv1x3x3(in_planes, out_planes, stride=1, t_stride=1, dilation=1):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, temporl_conv = False,
         norm_layer=None):
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, norm_layer=None):
        ...
        self.temporl_conv = [False, False, True, True]
        self.layer1 = self._make_layer(block, 64, layers[0], temporl_conv=self.temporl_conv[0])
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2, temporl_conv=self.temporl_conv[1])
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2, temporl_conv=self.temporl_conv[2])
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2, temporl_conv=self.temporl_conv[3])
    ...
    def _make_layer(self, block, planes, blocks, temporl_conv=False, stride=1):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, 
            temporl_conv=temporl_conv, norm_layer=norm_layer))
