...
from ops.utils import AverageMeter, map_sklearn
...
def train(train_loader, model, criterion, optimizer, epoch, log, tf_writer, 
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            ...
            output = ('Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t'
                ...
                data_time=data_time, forward_t=forward_t, backward_t=backward_t,
    ...
    output = ('Epoch: {}, mAP: {}'.format(epoch, np.around(ap_value, 3)))
    print(output)
    log.write(output + '\n')
    log.flush()

    tf_writer.add_scalar('loss/train', losses.avg, epoch)
    tf_writer.add_scalar('acc/train_map', ap_value, epoch)
